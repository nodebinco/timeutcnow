<script lang="ts">
	import type { City } from '$lib/types/timezone';
	import { Plus } from 'lucide-svelte';
	import { getTimezoneOffset, formatTimezoneOffset } from '$lib/utils/timezone-utils';

	interface Props {
		city: City;
		currentTime: Date;
		countryName?: string;
		onAdd?: () => void;
		class?: string;
	}

	let { city, currentTime, countryName, onAdd, class: className = '' }: Props = $props();

	const utcOffset = $derived(getTimezoneOffset(city.timezone, currentTime));
	const offsetDisplay = $derived(formatTimezoneOffset(utcOffset));
</script>

<div class="card bg-base-100 border border-base-300 hover:border-primary hover:shadow-md transition-all {className}">
	<div class="card-body p-4">
		<div class="flex items-center justify-between">
			<div class="flex-1">
				<h4 class="font-semibold text-sm">{city.name}</h4>
				<p class="text-xs text-base-content/60">{city.name}, {countryName || city.country}</p>
				<p class="text-xs text-base-content/60 mt-1">{offsetDisplay}</p>
			</div>
			{#if onAdd}
				<button
					type="button"
					class="btn btn-primary btn-circle btn-sm"
					onclick={(e) => {
						e.stopPropagation();
						onAdd();
					}}
					aria-label="Add city"
				>
					<Plus class="w-4 h-4" />
				</button>
			{/if}
		</div>
	</div>
</div>
